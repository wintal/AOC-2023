...
class Day16
{
  ...
  //      DoPart1("input.txt");
    ...
    static void DoPart1(string inputFile)
    {
        ...
        int[,] thisMap = new int[maxX - minX + 1, maxY - minY + 1];
        foreach (var entry in visited)
        {
            thisMap[entry.x - minX, entry.y - minY] = 1;
        }
        ...
        for (y = 0; y < maxY - minY + 1; y++)
        {
            ...
            for (x = 0; x < maxX - minX +1; x++)
            {
                if (thisMap[x, y] == 1)
                {
                    ...
                    if (!inWall)
                    {
                        if (y > 0 && thisMap[x, y - 1] == 1)
                        {
                            fromAbove = true;
                        }
                        if (y < (maxY - minY) && thisMap[x, y + 1] == 1)
                        {
                            fromBelow = true;
                        }
                    }
                ...
                else
                {
                    if (inWall)
                    {
                        ...
                        else if (y > 0 && x > 0 && thisMap[x - 1, y - 1] == 1)
                        {
                            if (fromBelow)
                            {
                                inOut++;
                        ...
                        else if(y < (maxY - minY) && thisMap[x - 1, y + 1] == 1)
                        {
                            if (fromAbove)
                            {
                                inOut++;
